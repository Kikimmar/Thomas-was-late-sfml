...
void Engine::populateEmitters(vector <Vector2f>& vSoundEmitters, int** arrayLevel)
{
	...
	FloatRect previousEmitter;  // keep track of the previous emitter so we don't make too many
		...
		for (int y = 0; y < (int)m_LM.getLevelSize().y; y++)
		{
			if (arrayLevel[y][x] == 2)   // fire is present
			{
									...
									TILE_SIZE, TILE_SIZE).intersects(previousEmitter))
				{
					...
					// Make a rectangle 6 blocks x 6 blocks, so we don't make any more emmiters
